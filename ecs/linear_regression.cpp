...
pdvd LinearRegression::loss(double reg) {
    double l = 0;
    vector<double> grad;
    for(int i = 0; i < n; i++) grad.push_back(0);
    ...
    for(int i = 0; i < n; i++) l += reg * this->w[i] * this->w[i];
    for(int i = 0; i < n; i++) {
...
vector<double> LinearRegression::predict(vector<double> testset, int len) {
    ...
    for(int i = testset.size() - this->n; i <= testset.size() - 1; i++) test.push_back(testset[i]);
    for(int i = 0; i < len; i++) {
        double score = 0;
...
void LinearRegression::show() {
    for (int i = 0; i < this->n; i++) cout << this->w[i] << endl;
}
